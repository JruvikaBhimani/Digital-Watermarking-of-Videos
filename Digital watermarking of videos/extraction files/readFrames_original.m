...
%import(demo1.fig);
    function readFrames_original(v)
    
    mkdir('F:\B.E. Project\bin\extraction\original');
    
    wait_bar=waitbar(0,'Converting  original Video to Frames.....','CreateCancelBtn',...
...
if getappdata(wait_bar,'canceling')
    ...
    for k = 1 : numFrames
        ...
        imwrite(mov(k).cdata, strcat('F:\B.E. Project\bin\extraction\original\frame',imagename));  %writes frame to image file
...
if ~flag
   rmdir('F:\B.E. Project\bin\extraction\original','s');
end

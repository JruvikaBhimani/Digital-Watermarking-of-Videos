...
%import(demo1.fig);
    function [flag]=readFrames(v)
    
    mkdir('F:\B.E. Project\bin\embedding\frames');
...
if getappdata(wait_bar,'canceling')
    ...
    for k = 1 : numFrames
        ...
        imwrite(mov(k).cdata, strcat('F:\B.E. Project\bin\embedding\frames\frame',imagename));  %writes frame to image file
...
if ~flag
   rmdir('F:\B.E. Project\bin\embedding\frames','s');
end
